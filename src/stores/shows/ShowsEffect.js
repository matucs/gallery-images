...
export default class ShowsEffect {
  ...
  static async requestPhoto(showId, section = 'hot', sort = 'viral', page = 0, window = 'all', showViral = true) {
    ...
    const response = await HttpUtility.get(endpoint, { 'showViral': showViral }, { 'headers': { 'Authorization': 'Client-ID 8b41ea2e3bdd188' } });

    if (response instanceof HttpErrorResponseModel) {
      return response;
    }

    return response.data.data.map((json) => new PhotoModel(json));
  }
